...
### CONTEXT RULE (where the HTML will live)

Canvas renders content in several places, and each one displays its own title above your HTML. Ask (or infer from the content) which context the fragment is for:

| Context | Canvas shows its own title? | Heading guidance |
|---------|------------------------------|------------------|
| **Content Page** (wiki page) | Yes (page title) | A styled banner (V02/V05) with an `<h1>` is fine when it is a designed header that intentionally restates the title. Otherwise start at `<h2>`. |
| **Assignment description** | Yes (assignment name, points, due date) | Never repeat the assignment name in the body. Start headings at `<h2>` (e.g., About, Format, Deliverables, Grading). Canvas already displays due date and points — don't duplicate them unless faculty ask. |
| **Syllabus** | Yes ("Syllabus" heading + course summary table) | Start at `<h2>`. Canvas appends its own auto-generated assignment summary below your content. **Required-format override:** in a course repo with `templates/outline/FORMAT.md`, the syllabus body IS the institution's Official Course Outline — follow that spec (plain hierarchical text, no theme) and copy the level-matched boilerplate (`templates/outline/boilerplate-undergrad.md` / `-grad.md`, per `level` in course.md) verbatim. Skip this file in restyle passes. It is a separate document from any designed front page in `pages/`. |
| **Discussion prompt** | Yes (topic title) | Start at `<h2>`. Keep it shorter — discussions render in a narrower column. |
| **Announcement** | Yes (announcement title) | Start at `<h2>`. Keep styling light; announcements are often read in email digests that strip inline styles entirely, so the text must read correctly unstyled. |

When faculty don't say which context, assume a Content Page. If the content is clearly an assignment (deliverables, points, due dates), apply the assignment rules and note that in your summary.
...
Faculty may say "I want my course to look like [website name]" or paste a URL. If your tool can fetch web pages, visit the URL and extract the dominant colors, font style (serif vs sans-serif), and overall feel, then map those to the closest theme as described in step 2 below. If you cannot visit external URLs, ask faculty to describe what they see:
...
- A Content Page should have at most one `<h1>` (typically inside a designed page header element). In assignment, syllabus, discussion, and announcement contexts, Canvas already renders the title — start at `<h2>` (see CONTEXT RULE).
...
**URL placeholder tokens.** Which placeholder to use depends on how the HTML reaches Canvas:

- **Paste workflow** (copying HTML into the RCE): use relative course paths with placeholders — `/courses/COURSE_ID/assignments/ASSIGNMENT_ID`, `/courses/COURSE_ID/pages/page-url-slug`, `/courses/COURSE_ID/files/FILE_ID/preview`. Faculty replace the IDs with real values from their course URLs.
- **Import workflow** (content packaged into a `.imscc` course export and imported via Canvas Settings → Import Course Content): use Canvas's official export tokens, which Canvas rewrites to real course URLs at import time:
  - `$WIKI_REFERENCE$/pages/page-url-slug` — link to a page in the same package
  - `$CANVAS_COURSE_REFERENCE$/assignments/...` — link to course-level content
  - `$IMS-CC-FILEBASE$path/to/file.jpg` — reference a file shipped inside the package's `web_resources/` folder
  
  These tokens survive import into any course, so packaged content is portable between course sections and semesters. Never paste token-style URLs into the RCE directly — they only work through the import pipeline.
...
- [ ] Logical heading order with no skipped levels; at most one `<h1>`, and none at all in assignment, syllabus, discussion, or announcement contexts (see CONTEXT RULE)
...
## Scaling Up: Bulk Creation via Course Packages

Generating one page at a time and pasting it into the RCE works for single pages, but whole courses (a semester of class pages, a full set of assignments with grading weights) should be **packaged and imported instead of pasted**.

If this skill is running in a local coding agent inside a **course repo** (a repo created from the [canvas-course-template](https://github.com/npuckett/canvas-course-template) template, or any project that includes its `tools/` folder), you can build a Canvas-importable `.imscc` package. In a course repo, also read the repo's `CLAUDE.md` and `style.md` — they define the course's structure and visual identity, and take precedence over this skill's defaults. If the repo has `templates/outline/FORMAT.md`, `syllabus.md` is the Official Course Outline with a required plain format and verbatim institutional boilerplate (see the Syllabus row in the CONTEXT RULE) — the themes and element library here do not apply to it:

1. Generate each page and assignment as a markdown file with front matter in a course source tree (`assignments/*.md`, `pages/*.md` — the body of each file is the Canvas HTML fragment, built with this skill's element library).
2. Define assignment groups and weights in `course.md`, and week-by-week module structure in `modules.md`.
3. Run `python3 tools/build_imscc.py <course-folder>` to produce the `.imscc` file.
4. Faculty import it once: Canvas → Settings → Import Course Content → "Canvas Course Export Package."

Identifiers in the package are stable across rebuilds, so regenerating and re-importing **updates** existing pages and assignments in place rather than duplicating them. Use `$WIKI_REFERENCE$`-style tokens (see Canvas Integration above) for all internal links in packaged content.

In a web chat without file tools, this workflow is unavailable — fall back to per-page HTML fragments as described in the Required Output Contract.
